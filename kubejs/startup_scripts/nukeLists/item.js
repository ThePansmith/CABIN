...
global.itemNukeList = [
    ...
    /^sophisticatedstorage:.*copper.*tier_upgrade$/,
    ...
    /^ad_astra:.*permafrost.*/,
    /^(ad_astra|everycomp|supplementaries):.*(aeronos|strophar|glacian).*/,

    // Tuff block duplicates (use Quark's variants instead)
    /^trials:(tuff_(bricks?|pillar|stairs|slab|wall)|(chiseled|polished)_tuff)/,
